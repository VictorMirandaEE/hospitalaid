...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--facility-id", type=str, help="The facility ID from HealthSites to import"
        )
        parser.add_argument(
            "--search-term",
            type=str,
            help="The search term to query for in the HealthSites " "facilities API",
        )

        parser.add_argument(
            "--insert-search-results",
            action="store_true",
            help="Indicates that search results should " "be inserted as the models",
        )
    ...
    def handle(self, *args, **options):
        ...
        if facility_id:
            logger.info(
                "Starting import of facility ID %s from HealthSites", facility_id
            )
            self.insert_facility_id(healthsites, facility_id)
        elif search_term:
            logger.info(
                "Searching HealthSites for facilities matching: %s", search_term
            )
            results = self.get_facilies_ids_from_search_response(
                healthsites.run_facility_search(search_term)
            )
            logger.debug("Search results: %s", results)
            if options["insert_search_results"]:
                logger.info(
                    "Search results from HealthSites will be inserted as models"
                )
                for facility_id, label in results.items():
                    logger.debug(
                        "Attempting to query HealthSites for details on %s (%s)",
                        label,
                        facility_id,
                    )
                    self.insert_facility_id(healthsites, facility_id)
    ...
    def build_model_from_facility_response(self, facility_response):
        ...
        imported_hospital.addr_postcode = facility_response["attributes"][
            "addr_postcode"
        ]
        imported_hospital.addr_housenumber = facility_response["attributes"][
            "addr_housenumber"
        ]
        imported_hospital.location_latitude = facility_response["centroid"][
            "coordinates"
        ][1]
        imported_hospital.location_longitude = facility_response["centroid"][
            "coordinates"
        ][0]
